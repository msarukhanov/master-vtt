...
const socialGraph = {
    ...
    getNodeHtml(x, y, name, id, type, kind) {
        ...
        const nodeData = isLoc
            ? { name: 'graph-go-to-loc', id: id, locName: name } // Что сделать (на карте)
            : { name: 'graph-recenter', id: id, kind: kind };   // Что сделать (в графе)

        return `
        <g class="graph-node" data-data='${JSON.stringify(nodeData)}' style="cursor:pointer">
            ${shape}
    ...
    popUp(id) {
        const item = currentSeason.characters.find(i=>i.id===id) || currentSeason.factions.find(i=>i.id===id) ||  data.locations.find(i=>i.id===id);
