...
const dices = {
    makeDices(main = dicesWrapper) {
        ...
        [['Chars', 'characters'], ['Factions', 'factions']].forEach(([label, type]) => {
            ...
            btn.onclick = () => this.rollGlobal(type);
            diceMenu.appendChild(btn);
        });

        const addBtn = createEl('button', 'dice-button add', '+');
        const playersContainer = createEl('div', 'dice-players');
        addBtn.onclick = () => this.addDicePlayer(playersContainer);
        ...
        rollBtn.onclick = () => this.processRolls();
