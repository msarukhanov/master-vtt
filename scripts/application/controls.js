...
const controls = {
    init() {
        ...
        const makeTab = (item) => {
            ...
            // group.dataset.data = JSON.stringify({type:'click',name:'left-menu-tab', data:item.code});
            ...
            item.tabs.forEach(tab => {
                ...
                linkInner.dataset.data = JSON.stringify({type:'click',name:'left-menu-tab-inner', data:tab.code});
        ...
        elementById('header-menu-icon').innerHTML = `<span class="icon">${assets.icons.menu}</span>`;
        elementById('left-menu-icon').innerHTML = `<span class="icon">${assets.icons.menu}</span>`;
        elementById('header-search-icon').innerHTML = `<span class="icon">${assets.icons.search}</span>`;
            ...
            if(listKeys.includes(tabCode)) {
                ...
                elementById('main-wrapper').innerHTML =
                    `<div class="wrapper editor-main">
                        <div id="editor-list">
                            <button class="short-list-button" data-data='{"type":"click","name":"editor-short"}'><<</button>
                            <button data-data='{"type":"click","name":"editor-save"}'>${l10n[lang].save}</button>
                            <button data-data='{"type":"click","name":"editor-save-avatars"}'>Export avatars</button>
                          
                            <div id="editorDataTypes"></div>
    ...
    createPopup(id, className, contentNode) {
        ...
        popup.dataset.data = JSON.stringify({type:'click',name:'popup-close', data:id});
        ...
        elementById('main-wrapper').appendChild(popup);
        return popup;
    }
};
